...
let DashboardService = class DashboardService {
    ...
    async getDashboardData(userId) {
        ...
        const prev_project_summary = {
            ...
            completed: prevProjects.filter((p) => p.status === 'COMPLETED' &&
                (p.delivery_date ? new Date(p.delivery_date) <= endOfPreviousMonth : p.updated_at <= endOfPreviousMonth)).length,
            in_progress: prevProjects.filter((p) => p.status === 'IN_PROGRESS' && p.updated_at <= endOfPreviousMonth).length,
            planning: prevProjects.filter((p) => p.status === 'PLANNING' && p.updated_at <= endOfPreviousMonth).length,
        };
        ...
        const total_expenses = Number(projectExpenses._sum.value || 0) +
        ...
        const prev_total_expenses = Number(prevProjectExpensesAgg._sum.value || 0) +
        ...
        const weekly_work_level = dailyMinutes.map((mins) => Number((mins / 60).toFixed(2)));
        return {
            project_summary,
