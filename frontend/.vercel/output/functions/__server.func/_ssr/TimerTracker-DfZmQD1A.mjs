...
import { r as toast } from "./axios-Dt9xUiMl.mjs";
import { C as fetchTasks, E as stopTimeEntry, T as startTimeEntry, w as fetchTimeEntries } from "./api-Beqz3ccz.mjs";
import { i as useQueryClient, n as useQuery } from "../_libs/tanstack__react-query.mjs";
import { c as Play, f as Pause, r as Stop } from "../_libs/gravity-ui__icons.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/TimerTracker-DfZmQD1A.js
...
function TimerProvider({ children }) {
	...
	const [totalPausedSeconds, setTotalPausedSeconds] = (0, import_react.useState)(0);
	const [pausedAt, setPausedAt] = (0, import_react.useState)(null);
	const [isLoaded, setIsLoaded] = (0, import_react.useState)(false);
	const queryClient = useQueryClient();
	(0, import_react.useEffect)(() => {
		if (!isLoaded) return;
		if (activeTimeEntryId) localStorage.setItem("timer_state", JSON.stringify({
			activeTaskId,
			activeTimeEntryId,
			activeStartTime,
			totalPausedSeconds,
	...
	(0, import_react.useEffect)(() => {
		fetchTimeEntries().then((entries) => {
			...
			if (running) {
				...
				if (savedState && savedState.activeTimeEntryId === running.id) {
					...
					let elapsed = 0;
					if (restoredIsRunning) elapsed = Math.max(Math.floor((Date.now() - start) / 1e3) - restoredTotalPaused, 0);
					else elapsed = Math.max(Math.floor(((restoredPausedAt || Date.now()) - start) / 1e3) - restoredTotalPaused, 0);
		...
		}).catch((err) => console.error("Error fetching running timer:", err)).finally(() => setIsLoaded(true));
	}, []);
	(0, import_react.useEffect)(() => {
		let interval;
		if (isRunning && activeStartTime) interval = setInterval(() => {
			const start = new Date(activeStartTime).getTime();
			const totalElapsed = Math.max(Math.floor((Date.now() - start) / 1e3) - totalPausedSeconds, 0);
	...
	const toggleTimer = async (explicitTaskId) => {
		if (isRunning) {
			setIsRunning(false);
			...
			if (activeTimeEntryId && activeStartTime) {
				...
				const secondsInThisPause = Math.max(Math.floor((Date.now() - currentPausedAt) / 1e3), 0);
	...
	const startTimer = (explicitTaskId) => {
		if (!isRunning) toggleTimer(explicitTaskId);
	};
	const pauseTimer = () => {
		if (isRunning) toggleTimer();
	};
	const resetTimer = async () => {
		if (activeTimeEntryId && activeStartTime) try {
			const currentPausedAt = pausedAt || Date.now();
			const finalTotalPaused = totalPausedSeconds + (isRunning ? 0 : Math.max(Math.floor((Date.now() - currentPausedAt) / 1e3), 0));
			const start = new Date(activeStartTime).getTime();
			const actualElapsedSeconds = Math.max(Math.floor((Date.now() - start) / 1e3) - finalTotalPaused, 0);
			await stopTimeEntry(activeTimeEntryId, void 0, new Date(start + actualElapsedSeconds * 1e3).toISOString());
...
function TimerTracker({ variant = "sidebar" }) {
	...
	if (variant === "dashboard") return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		className: "w-full bg-primary/50 border border-primary rounded-[24px] space-y-4 px-6 py-4 flex flex-col items-center justify-between text-secondary",
		children: [
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		className: "w-full bg-primary/50 border border-primary rounded-[24px] px-4 py-6 space-y-4 flex flex-col items-center justify-between text-secondary",
		children: [
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
